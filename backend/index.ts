...
app.post('/api/qr/generate', qrLimiter, requireAuth(), (req: any, res: Response) => {
...
app.post('/api/users/sync', validateUserSync, (req: Request, res: Response) => {
  ...
  db.get('SELECT * FROM Users WHERE clerk_id = ?', [clerk_id], (err, any, row: any) => {
    ...
    if (row) {
      ...
      stmt.run([clerk_id, email, name, role], function(err: any) {
        if (err) return res.status(500).json({ error: err.message });
...
app.put('/api/users/:clerk_id/role', validateRoleUpdate, (req: Request, res: Response) => {
  const { role } = req.body;
  db.run('UPDATE Users SET role = ? WHERE clerk_id = ?', [role, req.params.clerk_id], function(err: any) {
    if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/users/sync-secret', requireAuth(), (req: any, res: Response) => {
  const clerk_id = req.auth.userId;

  db.get('SELECT qr_secret FROM Users WHERE clerk_id = ?', [clerk_id], (err: any, row: any) => {
    if (err) return res.status(500).json({ error: err.message });
    ...
    if (row && row.qr_secret) {
      ...
      db.run('UPDATE Users SET qr_secret = ? WHERE clerk_id = ?', [newSecret, clerk_id], function(err: any) {
        if (err) return res.status(500).json({ error: err.message });
...
// --- VEHICLES API ---
app.get('/api/vehicles/:clerk_id', (req: Request, res: Response) => {
  db.all('SELECT * FROM Vehicles WHERE owner_clerk_id = ?', [req.params.clerk_id], (err: any, rows: any[]) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});

app.post('/api/vehicles', validateVehicle, (req: Request, res: Response) => {
  const { plate, owner_clerk_id, make, model } = req.body;
  db.run('INSERT INTO Vehicles (plate, owner_clerk_id, make, model) VALUES (?, ?, ?, ?)', 
    [plate.toUpperCase(), owner_clerk_id, make, model], function(this: any, err: any) {
    if (err) return res.status(500).json({ error: err.message });
...
app.delete('/api/vehicles/:id', (req: Request, res: Response) => {
  db.run('DELETE FROM Vehicles WHERE id = ?', [req.params.id], function(err: any) {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ success: true });
  });
});
...
app.get('/api/zones', (req: Request, res: Response) => {
  db.all('SELECT * FROM Zones', (err: any, rows: any[]) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});

app.post('/api/zones', validateZone, (req: Request, res: Response) => {
  const { name, total_capacity } = req.body;
  db.run('INSERT INTO Zones (name, total_capacity, current_occupancy) VALUES (?, ?, 0)', [name, total_capacity], function(this: any, err: any) {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ id: this.lastID, name, total_capacity, current_occupancy: 0 });
  });
});

app.put('/api/zones/:id', validateZoneUpdate, (req: Request, res: Response) => {
  const { name, total_capacity } = req.body;
  db.run('UPDATE Zones SET name = ?, total_capacity = ? WHERE id = ?', [name, total_capacity, req.params.id], function(err: any) {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ success: true });
  });
});
...
app.delete('/api/zones/:id', (req: Request, res: Response) => {
  db.run('DELETE FROM Zones WHERE id = ?', [req.params.id], function(err: any) {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ success: true });
  });
});
...
// --- EVENTS API (Scanner) ---
app.post('/api/events/scan', scanLimiter, requireAuth(), validateScanEvent, (req: Request, res: Response) => {
    ...
    db.get('SELECT qr_secret FROM Users WHERE clerk_id = ?', [payload.userId], (err: any, userRow: any) => {
      if (err) return res.status(500).json({ error: err.message });
      ...
      db.get('SELECT event_type FROM AccessEvents WHERE vehicle_plate = ? ORDER BY timestamp DESC LIMIT 1', [vehicle_plate], (err: any, lastEvent: any) => {
          if (err) return res.status(500).json({ error: err.message });
          
          const isEntry = !lastEvent || lastEvent.event_type === 'EXIT';

          db.get('SELECT current_occupancy, total_capacity FROM Zones WHERE id = ?', [zone_id], (err: any, zone: any) => {
              if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/events/last/:plate', (req: Request, res: Response) => {
    db.get('SELECT * FROM AccessEvents WHERE vehicle_plate = ? ORDER BY timestamp DESC LIMIT 1', [req.params.plate], (err: any, row: any) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/events', validateManualEvent, (req: Request, res: Response) => {
